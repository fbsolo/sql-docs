...
## Remarks  
Use `APP_NAME` to distinguish between different applications, as a way to perform different actions for those applications. For example, `APP_NAME` can distinguish between different applications which will allow for a different date format for each application. It can also allow for the return of an informational message to certain applications.
